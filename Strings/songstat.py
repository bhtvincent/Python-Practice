...
def count_words(song):
    ...
    for each_word in words:
        ...
        tally[each_word] = tally.get(each_word, 0) + 1 #if it doesnt exist, set it to 0
    return tally
...
def report(word_count):
    ...
    ##for every_word in sorted(word_count):
    ##   print(f"{every_word}: {word_count[every_word]}")

    print(f"{sum(word_count.values())}")
    print(f"{word_count.get}")
